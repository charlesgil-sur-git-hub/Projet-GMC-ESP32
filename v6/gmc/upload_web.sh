...
if [ "$#" -ne 2 ]; then
    echo "Usage: ./upload.sh [CHIP] [PORT]"
    echo "Exemples :"
    echo "  ./upload.sh esp32s3 /dev/ttyUSB0"
    echo "  ./upload.sh esp32c6 /dev/ttyACM0"
    exit 1
fi
...
echo "Cible : $CHIP sur $PORT - Connexion..."
...
$ESPTOOL_BIN --chip "$CHIP" --port "$PORT" --baud 115200 write_flash 0x290000 data.bin
